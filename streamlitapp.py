...
def drawboundingbox(img, boxes,pred_cls, rect_th=2, text_size=1, text_th=2):
    ...
    for i in range(len(boxes)):
        ...
        cv2.putText(img,pred_cls[i], (int(boxes[i][0]), int(boxes[i][1])-5),  cv2.FONT_HERSHEY_SIMPLEX, text_size, class_color_dict[pred_cls[i]],thickness=text_th)
    return img
...
if bytesObj or url:
    ...
    img = drawboundingbox(img, data_dict['boxes'], data_dict['classes'])
    # st.pyplot()
    fig, ax = plt.subplots()
    ax.imshow(img)
    ax.axis('off')
